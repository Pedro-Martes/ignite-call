...
import { CalendarAction, CalendarBody, CalendarContainer, CalendarDay, CalendarHeader, CalendarTitle } from "./style";
    ...
    const calendarWeeks = useMemo(() => {
        ...
        const calendarDays = [
            ...
            ...daysInMonthArray.map((date) => {
                return {
                    date, disabled: date.endOf('day').isBefore(new Date()) || blockedDates.blockedWeekDays.includes(date.get('day')) || blockedDates.blockedDates.includes(date.get('date')),
                }
            }),
    ...
    return (
        <CalendarContainer>
            ...
            <CalendarBody>
                <thead>
                    <tr>
                        {shortWeekDays.map((shortWeek => <th key={shortWeek}>{shortWeek}.</th>))}
                    </tr>
                </thead>
                                ...
                                {days.map(({ date, disabled }) => {
                                    return (
                                        <td key={date.toString()}>
                                            <CalendarDay onClick={() => onDeteSelected(date.toDate())} disabled={disabled}>
                                                {date.get('date')}
                                            </CalendarDay>
                                        </td>
                                    )

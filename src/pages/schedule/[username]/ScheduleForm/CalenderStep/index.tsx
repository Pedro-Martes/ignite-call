...
import { Container, TimerPicker, TimerPickerHeader, TimerPickerItem, TimerPickerList } from "./styles";
...
interface CalendarStepProps {
  ...
  export function CalendarStep({ onSelectDateTime }: CalendarStepProps) {
    ...
    const dateNumber = selectedDate ? dayjs(selectedDate).format('DD [de] MMMM') : null

    const selectDateWithoutTime = selectedDate ? dayjs(selectedDate).format('YYYY-MM-DD') : null

    const { data: availability } = useQuery<Availablility>(['availability', selectDateWithoutTime], async () => {
